@component('mail::layout')
    ...
    {{-- Body --}}
    <p style="margin-top:25px;margin-bottom:25px;font-size:22px;letter-spacing:-1px;color:#757575">{!! $reservation->propertyInfo->name !!}!</p>
    <h1 style="font-size:20px;font-weight:bold;margin-bottom:17px;line-height:1.4">{!! __('messages.You have a booking cancellation for the') !!} {!! $reservation->propertyInfo->name !!}!</h1>
        ...
        <tbody>
            <tr style="border-top:1px solid #dbdee3">
                <td align="left" style="padding:5px 0">
                    <p style="font-size:15px;font-weight:bold;line-height:1.43;letter-spacing:0.27px;color:rgba(0,0,0,0.6)">{{ $reservation->propertyInfo->min_bed }} {{ $reservation->propertyInfo->min_bed = 1 ? ' Bedroom' : ' Bedrooms'}} {{ $reservation->propertyInfo->name }}</p>
